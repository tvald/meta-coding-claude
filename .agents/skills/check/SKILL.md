...
Run the **maintenance loop** (`readme/meta/process/loops.md#maintenance-loop`) in the
curator role (`readme/meta/agents/roles.md#curator`) — delegate it to a separate agent
when the main context is already loaded with other work. Read the loop now and execute
every step, scoped by any arguments given at invocation:
...
2. Budget enforcement per `readme/meta/knowledge/management.md` — including *Next
   steps* overflow to the backlog and dead-end graduation to `derived.md` *Gotchas*.
...
dated maintenance retro entry to `readme/log/retros.md` (this entry is the scheduler
for the next run).
